...
class EnvironmentCreator(object):
    def __init__(self, num_agents=10, env_size=5.):
        self.num_agents = num_agents
        self.env_size = env_size
        rospack = rospkg.RosPack()
        self.actor_pkg_path = rospack.get_path("menge_gazebo_worlds")
        world_name = "menge_empty.world"
        parser = etree.XMLParser(remove_blank_text=True)
        self.tree = etree.parse(self.actor_pkg_path+'/worlds/'+world_name, parser)
    ...
    def run(self):
        ...
        for item in range(self.num_agents):
            ...
            plugin = Element("plugin", name="actor_plugin", filename="libInstancedActorPlugin.so")
        ...
        self.tree.write(self.actor_pkg_path+'/worlds/ped_square1.world', pretty_print=True, xml_declaration=True, encoding="utf-8")
