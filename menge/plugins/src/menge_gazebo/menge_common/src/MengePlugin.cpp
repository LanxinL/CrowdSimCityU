...
int MengePlugin::simMain( SimulatorDBEntry * dbEntry, const std::string & behaveFile, const std::string & sceneFile, const std::string & outFile, const std::string & scbVersion, const std::string & dumpPath ) {
  ...
  std::cout << "agentCount " << agentCount << "\n";
...
/*  auto skelAnims = _actor->SkeletonAnimations();
  ...
  */{
    ...
    std::cout << "@llx trajectoryInfo" << this->trajectoryInfo ;
...
bool MengePlugin::updateAgent(const Agents::BaseAgent* agt)
{
 ...
 //     actor->GetWorldPose().Ign().Pos()).Length();
  ...
  // pose.Set(30.787, 32.96, 0, 1.5707, 0, 2.90923);
...
void MengePlugin::updateGoals()
{
    ...
    bool mdlIsNull = (mdl==nullptr);
    std::cout << "################## goalLlx " << mdlIsNull << " \n" ;
    ...
    for(size_t i=0;i<gs->size();i++)
    {
        ...
        // cg->setCenter(30.787, 32.96); // not be used, mdl==nullptr
    }
}
void MengePlugin::simUpdate(float simTime)
{
  std::cout << "*** into simUpdate " ;
	_mengeView.pause();
	updateAgents();
  std::cout << "*** into updateGoals1 \n" ; 
	updateGoals();
  std::cout << "*** into updateGoals2 \n" ;
	_mengeView.resume();
...
#ifdef USE_ACTOR 
                std::cout << "*** into controlActorAgents"; // @llx into this
                  controlActorAgents();
#endif
                std::cout << "*** into updateAgents";
                  updateAgents();
	     }
